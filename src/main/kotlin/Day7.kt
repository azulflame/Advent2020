...
{
	return Pair(day7part1(ListHolder.day7), day7part2(ListHolder.day7))
}
...
{
	val firstMap = list.map {
		it.split(Regex(" bags contain "))[0] to Regex("([0-9]+ [a-z ]+) bag").findAll(it.split(Regex(" bags contain "))[1])
			.map { a -> a.groupValues.drop(1).first() }.toList()
	}.toMap()
...
fun dfs(graph: Map<String, List<String>>, target: String): Int
{
	...
	return graph[target]!!.sumOf { x ->
		Regex("([0-9]+)").find(x)!!.value.toInt() * (1 + dfs(graph, Regex(" ([a-z ]+)").find(x)!!.groupValues[1]))
	}
}